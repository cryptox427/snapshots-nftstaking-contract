...
class User {
    ...
    async unstakeNFTToken(admin) {
        ...
        const vault = admin.vaultPubkey;
        let vaultObject = await this.program.account.vault.fetch(vault);
        let toTokenAccount = await vaultObject.nfts.find(async (nft) => {
            let vaultNftAccountInfo = await nftToken.getAccountInfo(nft);
            if(vaultNftAccountInfo.mint.toBase58() == mint.toBase58()) {
                return nft;
            }
        })
